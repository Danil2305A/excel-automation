...
1. Несовместимость версий glibc. Приложение на момент июнь 2026 требует как минимум glibc 2.34. 
Выполните команду:
```bash
ldd --version
```
Если в системе версия glibс < 2.34, программа не сможет запуститься. В таком случае нужно менять в [cd.yaml](.github/workflows/cd.yaml) задачу 
